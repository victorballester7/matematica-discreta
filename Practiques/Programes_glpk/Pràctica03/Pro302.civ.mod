...
var y{magatzems} binary;
var z{magatzems};
minimize cost_total:sum{i in magatzems, j in clients} x[i,j]*cost[i,j]+sum{i in magatzems} z[i];

s.t.
r_demanda{j in clients}: sum{i in magatzems} x[i,j]=demanda[j];
r_produccio{i in magatzems}: sum{j in clients} x[i,j]<=capacitat[i]*y[i];
r_z{i in magatzems}: z[i]=cost_fix[i]*y[i];
r_nombre_de_magatzems: sum{i in magatzems} y[i]=1;
